...
void MainWindow::realtimeDataSlot() {
    ...
    {
        // add data to lines:
        ui->plot->graph(0)->addData(key, qSin(key)+QRandomGenerator::global()->generate()/(double)RAND_MAX*1*qSin(key/0.3843));
        ui->plot->graph(1)->addData(key, qCos(key)+QRandomGenerator::global()->generate()/(double)RAND_MAX*0.5*qSin(key/0.4364));
